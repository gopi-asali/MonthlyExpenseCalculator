...
public class dbWriterClass extends JdbcDaoSupport implements ItemWriter<String[]> {
	...
	public void write(final List<? extends String[]> values) throws Exception {
		...
		Date mydate = new Date(dateValue);
		String procDate = new SimpleDateFormat("ddMMyy").format(mydate);
		System.out.println("Procedure date format is: " + procDate);
		CallableStatement callableStatement = getConnection().prepareCall("{call P_SHARES('" + procDate + "')}");
		callableStatement.execute();
		List<Map<String, Object>> result = this.getJdbcTemplate().queryForList(
				"SELECT RPAD(S_NO,4,' ') AS S_NO, RPAD(NAME,30,' ') AS NAME, RPAD(AMOUNT,10,' ') AS AMOUNT,RPAD(CALC_DATE,10,' ') AS CALC_DATE FROM BALANCE WHERE TO_DATE(CALC_DATE)='"
						+ dateValue + "'");
	...
	public List<Map<String, Object>> getResult() {
		...
		if (dateValue.equals("ALL"))
			result = this.getJdbcTemplate().queryForList(
					"SELECT RPAD(S_NO,4,' ') AS S_NO, RPAD(NAME,30,' ') AS NAME, RPAD(AMOUNT,10,' ') AS AMOUNT,RPAD(CALC_DATE,10,' ') AS CALC_DATE FROM BALANCE");
		else
			result = this.getJdbcTemplate().queryForList(
					"SELECT RPAD(S_NO,4,' ') AS S_NO, RPAD(NAME,30,' ') AS NAME, RPAD(AMOUNT,10,' ') AS AMOUNT,RPAD(CALC_DATE,10,' ') AS CALC_DATE FROM BALANCE  WHERE TO_DATE(CALC_DATE)='"
							+ dateValue + "'");
	...
	public void setSql(String sql) {
		List<Map<String, Object>> values = this.getJdbcTemplate().queryForList(
				"SELECT COLUMN_NAME FROM ALL_TAB_COLUMNS WHERE TABLE_NAME='SHARES' AND COLUMN_NAME!='C_DATE' ORDER BY COLUMN_ID");
